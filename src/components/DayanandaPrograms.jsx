...
export default function DayanandaPrograms({ onEnquiryClick }) {
    ...
    const fetchCourses = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const uniCourses = data.filter(c => c.university === 'Dhayananth Sagar University Bangalore');
    ...
    <section className="bg-gradient-to-br from-white via-white to-purple-50 py-20 px-6 md:px-12 lg:px-24">
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        ...
        <h2 className="text-4xl md:text-5xl font-extrabold text-[#1f153a] mb-10 text-center leading-tight tracking-tight">
          Dayananda <br /> University Programes
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
          {displayPrograms.map((program, index) => (
            ...
            >
              ...
              <button className="mt-8 self-start border border-purple-300 text-purple-500 hover:bg-purple-50 font-semibold text-xs px-4 py-2 rounded flex items-center gap-2 transition-colors" onClick={() => onEnquiryClick(program.title, 'Dayananda Sagar University')}>
