    ...
    const fetchCourses = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const uniCourses = data.filter(c => c.university === 'Amity University');
        ...
        {dbPrograms.length > 0 && (
          <div className="mt-20">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">
                <span className="text-[#1c2d76]">More </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {dbPrograms.map((program, index) => (
                ...
                >
                  ...
                  <button className="mt-8 self-start border border-purple-300 text-purple-500 hover:bg-purple-50 font-semibold text-xs px-4 py-2 rounded flex items-center gap-2 transition-colors" onClick={() => onEnquiryClick(program.title, 'Amity University')}>
